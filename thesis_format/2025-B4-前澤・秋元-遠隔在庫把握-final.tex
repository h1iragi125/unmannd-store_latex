...
% 和題("\\"で任意の改行位置を設定可能)
\titleJP{画像処理および距離センサを用いた\\[-1.0em]遠隔在庫把握システムの構築}
% 英題（"\\"で任意の改行位置を設定可能）
\titleEN{Development of a Remote Inventory Monitoring System \\ Using Image Processing and Distance Sensors}
...
% PDFメタデータの設定(任意)
\hypersetup{%
	pdftitle={画像処理および距離センサを用いた遠隔在庫把握システムの構築},				% PDFタイトル
	pdfsubject={Development of a Remote Inventory Monitoring System Using Image Processing and Distance Sensors}					% PDFサブタイトル
	pdfauthor={前澤 柊飛, 秋元 大生}				% 著者
	pdfkeywords={Sensors,Stores,店舗,Remote,遠隔,Monitoring,在庫把握}		% キーワード
}
...
\begin{document}
	...
	% ---　序論(chapter1)
	\include{./contents/chapter1/introduction}
	
	% ---　画像処理による在庫検出(chapter2)
	\include{./contents/chapter2/image_processing}
	
	% ---　超音波センサによる在庫検出(chapter3)
	\include{./contents/chapter3/dis_sensor}
	
	% --- デプスカメラによる在庫検出(chapter4)
	\include{./contents/chapter4/depth_camera}

	% --- LINE BOTを用いた遠隔在庫把握(chapter5)
	\include{./contents/chapter5/line_bot}

	% --- 結論(chapter6)
	\include{./contents/chapter6/conclusion}
	
	\backmatter			% \chapter毎に\phantomsectionの設定が必要(これでハイパーリンクを設定)
	
	% --- 参考文献
	% --- thebibliography環境->preambleで"\usepackage[thebibenv]{thesismacro}"を指定
	\include{./contents/reference/reference}
	% --- biblatex+biber->preambleで"\usepackage[usebiber]{thesismacro}"&"\addbibresource{.bib}"を指定
	%\printbibliography	% 文献リストの挿入
	
	% --- 謝辞
	\include{./contents/sub/acknowledgement}
	
	\mainmatter			% \phantomsectionは不要
	% ---------- APPENDIX ----------
	\appendix	%　以下は付録
	% --- 関連プログラム
	\include{./contents/appendix/source_code/print_sourcecode}
	% --- 原稿非掲載データ
	% \include{./contents/appendix/data/raw_data}
